...
const addTodo = async (todoText) => {
    try {
        isLoading.value = ref(true)
        await  todoStore.addTodos(todoText)
        isLoading.value = ref(false)
    } catch (error) {
...
const editStatus = async (todoId,todoData) => {
    try {
        ...
        isLoading.value = ref(true)
        await todoStore.editTodos(updateData,todoId)
        isLoading.value = ref(false)
    } catch (error) {
        console.log('error', error)
    }
...
const deleteTodo = async (todoId) => {
    try {
        isLoading.value = ref(true)
        await todoStore.removeTodos(todoId)
        await  todoStore.loadTodos()
        isLoading.value = ref(false)
    } catch (error) {
        console.log('error', error)
...
const changeStatus = async (event, todoId) => {
    ...
    try {
        ...
        await todoStore.editTodos({status: 'Done'} , todoId)
        } else {
        await todoStore.editTodos({status: 'Donig'} , todoId)       
        }
    await  todoStore.loadTodos()
...
<template>  
    <div>
        ...
        <Loading v-if="isLoading.value"></Loading>
        <div class="my-3">          
        ...
        <div class="flex items-center justify-between my-3" v-for="todo in filterTofoList" :key="todo.id">
            <div>
